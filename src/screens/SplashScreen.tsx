...
const SplashScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    ...
    Animated.timing(slideAnim, { toValue: 0, duration: 1200, delay: 400, useNativeDriver: true }).start();

    const timer = setTimeout(() => navigation.replace('MainApp'), 3500);
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#0a0e27', '#1a1a2e', '#16213e']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
      ...
      <Animated.View style={[styles.bottomContent, { transform: [{ translateY: slideAnim }] }]}>
        <LinearGradient colors={['rgba(229, 9, 20, 0.3)', 'rgba(229, 9, 20, 0.1)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.gradientBox}>
...
const styles = StyleSheet.create({
  ...
  subtitleText: { fontSize: 32, fontWeight: '300', color: '#fff', letterSpacing: 6, marginTop: 8 },
  bottomContent: { position: 'absolute', bottom: 40, width: '80%', alignSelf: 'center' },
