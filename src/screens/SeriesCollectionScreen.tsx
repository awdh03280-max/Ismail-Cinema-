/**
 * SeriesCollectionScreen — shows all movies in a franchise/universe
 * (Marvel, DC, Harry Potter, etc). Route params: { seriesCollectionId: string }
 */
...
const SeriesCollectionScreen = ({ route, navigation }: any) => {
  const { seriesCollectionId } = route.params as { seriesCollectionId: string };
  const collection: SeriesCollection | undefined = SERIES_COLLECTIONS.find(
    (c) => c.id === seriesCollectionId,
  );
  ...
  const loadMovies = useCallback(async () => {
    ...
    try {
      const [raw, favs] = await Promise.all([
        collection.source.type === 'tmdbCollection'
          ? getMoviesByCollectionId(collection.source.collectionId)
          : getMoviesByCompanyId(collection.source.companyId),
        getFavorites(),
      ]);
      setMovies(filterMovies(raw));
      setFavorites(new Set(favs.map((f) => f.imdbID)));
    } catch (e) {
      console.error('[SeriesCollectionScreen] load error:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
...
export default SeriesCollectionScreen;
