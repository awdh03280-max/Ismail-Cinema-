...
- **2026-07-18**: Four critical fixes: (1) Android Profile crash — removed shared JSX variable pattern (`playerScreen`, `actorProfileScreen`, `makeSocialScreens`); all `Stack.Screen` elements are now inlined directly inside each `Stack.Navigator` as required by the native stack renderer on Android. (2) Video playback / ad redirects — `StreamEmbed.native.tsx` now uses `onShouldStartLoadWithRequest` to block main-frame navigations to non-embed domains, plus injected JS kills `window.open()` popups. (3) True fullscreen player — `PlayerScreen` moved from each tab sub-stack to the root `Stack.Navigator` (above `BottomTabNavigator`); tab bar is hidden during playback; landscape lock/restore remains in `PlayerScreen.tsx`. (4) TMDB Arabic — added `language: 'ar'` as a default axios param on the TMDB client so all movie, TV, actor and search requests return Arabic content. TypeScript: 0 errors. Expo Doctor: 16/16 checks passed.
